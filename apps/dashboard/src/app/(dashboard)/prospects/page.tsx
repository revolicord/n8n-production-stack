...
export default async function ProspectsPage({
  ...
  const view = await getLeadsForMonth({ tenantId: tenant.id, year, month });
  ...
  const switcherOptions = [
    { label: prevLabel, href: `/prospects?year=${prevYear}&month=${prevMonth}` },
    { label: monthLabel.slice(0, 3), href: `/prospects?year=${year}&month=${month}`, active: true },
  ];
  ...
  return (
    <div className="px-6 py-5">
      ...
      <div className="overflow-x-auto">
        <ProspectsTable columns={view.columns} leads={view.leads} />
      </div>
    </div>
  );
}
