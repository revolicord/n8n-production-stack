...
import { adminSecurity, doc, zodDoc } from '../../lib/openapi.js';
...
export default async function turnCompletedRoute(app: FastifyInstance): Promise<void> {
  ...
  app.post(
    '/admin/turn-completed',
    doc({
      tags: ['admin/misc'],
      summary: 'Callback de n8n al cerrar un turno',
      description:
        'Persiste métricas del turno (tokens, costo, modelo), libera el lock de turno y, si llegaron ' +
        'mensajes durante el procesamiento, encola un drain inmediato. Responde 204.',
      security: adminSecurity,
      body: zodDoc(TurnCompletedSchema),
    }),
    async (req, reply) => {
