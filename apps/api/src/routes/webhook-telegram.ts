...
export default async function webhookTelegramRoute(app: FastifyInstance): Promise<void> {
  ...
  app.post('/webhook/telegram', async (req, reply) => {
    if (
      ...
      !verifyMcToken(req.headers['x-telegram-bot-api-secret-token'], config.TELEGRAM_WEBHOOK_SECRET)
    ) {
    ...
    if (action === 'pause') {
      ...
      await resolveNotification(getDb(), { id: notification.id, resolvedBy: `telegram:${actor}` });
