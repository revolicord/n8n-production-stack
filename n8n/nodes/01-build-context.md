...
// Etapa actual del lead (viene dentro de subscriber en el payload del API)
const currentStage = body.subscriber.lead_stage ?? body.subscriber.metadata?.stage ?? 'nuevo';
...
- `lead_stage` viene dentro de `body.subscriber.lead_stage` en el payload (tabla `lead_stages`). Si no existe, cae al fallback `subscriber.metadata.stage` para compatibilidad hacia atrás.
